...
- (void)createUI {
    ...
    redDotBtn.frame = CGRectMake(SCALE_SIZE(15), (70 - 45) / 2, 40, 45);
...
- (void)messgeInfoWithModel:(JKMessageModel *)model {
    if ([model.messageType isEqualToString:@"install"]) {
        ...
        self.imgV.image = [UIImage imageNamed:@"ic_alarm_notification"];
        self.titleLb.text = @"报警通知";
...
- (void)clickCell:(UITapGestureRecognizer*)tap {
    if ([_delegate respondsToSelector:@selector(cellClick:)]) {
        [_delegate cellClick:self.mModel];
    }
}
