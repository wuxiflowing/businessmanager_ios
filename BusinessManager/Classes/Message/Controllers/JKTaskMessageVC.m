...
- (void)getTaskInfoList:(NSString *)tskID withMessageType:(NSString *)type {
    ...
    [[JKHttpTool shareInstance] GetReceiveInfo:nil url:urlStr successBlock:^(id responseObject) {
        if (responseObject[@"success"]) {
            ...
            NSString *txtMessageState = responseObject[@"data"][@"txtMessageState"];
            if ([type isEqualToString:@"install"]) {
                if ([taskState isEqualToString:@"ready"]) {
                    JKInstallInfoVC *iwiVC = [[JKInstallInfoVC alloc] init];
                    ...
                    JKInstallInfoVC *iiiVC = [[JKInstallInfoVC alloc] init];
                ...
                if ([taskState isEqualToString:@"ready"]) {
                    JKRepaireInfoVC *rwiVC = [[JKRepaireInfoVC alloc] init];
                ...
                if ([txtMessageState isEqualToString:@"toAudited"]) {
                    JKRecycePendInfoVC *riVC = [[JKRecycePendInfoVC alloc] init];
                    riVC.recyceType = JKRecyceCheck;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"auditPass"]) {
                    JKRecycePendInfoVC *riVC = [[JKRecycePendInfoVC alloc] init];
                    riVC.recyceType = JKRecyceAuditPass;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"auditReject"]) {
                    JKRecycePendInfoVC *riVC = [[JKRecycePendInfoVC alloc] init];
                    riVC.recyceType = JKRecyceAuditReject;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"magAuditPass"]) {
                    JKRecycePendInfoVC *riVC = [[JKRecycePendInfoVC alloc] init];
                    riVC.recyceType = JKRecyceMagAuditPass;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"magAuditReject"]) {
                    JKRecycePendInfoVC *riVC = [[JKRecycePendInfoVC alloc] init];
                    riVC.recyceType = JKRecyceMagAuditReject;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"receive"]) {
                    JKRecyceInfoVC *riVC = [[JKRecyceInfoVC alloc] init];
                    riVC.recyceType = JKRecyceIng;
                    riVC.tskID = tskID;
                    [self.navigationController pushViewController:riVC animated:YES];
                } else if ([txtMessageState isEqualToString:@"finish"]) {
                    JKRecyceInfoVC *riVC = [[JKRecyceInfoVC alloc] init];
