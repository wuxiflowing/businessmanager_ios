...
@interface JKMessageVC () <UITableViewDelegate, UITableViewDataSource, JKMessgaCellDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray *totalMessageArr;
...
- (NSMutableArray *)totalMessageArr {
    if (!_totalMessageArr) {
        _totalMessageArr = [[NSMutableArray alloc] init];
    }
    return _totalMessageArr;
}
...
- (void)dropDownRefresh {
    MJRefreshNormalHeader *header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(refreshMessageList)];
...
- (void)getMyTaskMessageList {
    ...
    [[JKHttpTool shareInstance] GetReceiveInfo:nil url:urlStr successBlock:^(id responseObject) {
        ...
        if (responseObject[@"success"]) {
            [self.totalMessageArr removeAllObjects];
            for (NSDictionary *dict in responseObject[@"data"]) {
                JKMessageModel *model = [JKMessageModel mj_objectWithKeyValues:dict];
                [self.totalMessageArr addObject:model];
            }
        }
...
#pragma mark -- JKMessgaCellDelegate
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.totalMessageArr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JKMessageModel *model = self.totalMessageArr[indexPath.row];
