...
@interface JKRecyceCheckVC () <UITableViewDelegate, UITableViewDataSource, JKRecyceCheckCellDelegate>
{
    NSInteger _page;
}
...
@implementation JKRecyceCheckVC
...
- (void)getRecyceTaskList:(NSInteger)page {
    NSString *urlStr = [NSString stringWithFormat:@"%@/RESTAdapter/mytask/%@/list/%@/%@?page=%ld",kUrl_Base, [JKUserDefaults objectForKey:@"loginid"],@"recycling",@"check",(long)page];
    ...
    [[JKHttpTool shareInstance] GetReceiveInfo:nil url:urlStr successBlock:^(id responseObject) {
        ...
        if (responseObject[@"success"]) {
            ...
            for (NSDictionary *dict in responseObject[@"data"]) {
                ...
                model.txtCenMagName = dict[@"txtCSMembName"];
                model.txtFarmerName = dict[@"txtFarmerName"];
                model.txtHK = dict[@"txtHK"];
                model.txtFarmerPhone = dict[@"txtFarmerPhone"];
                model.txtStartDate = dict[@"txtStartDate"];
                model.tarCustomerReson = dict[@"tarCustomerReson"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JKRecyceCheckCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if(!cell){
        cell = [[JKRecyceCheckCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    
    cell.recyceType = JKRecyceCheck;
