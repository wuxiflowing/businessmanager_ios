...
@interface JKInstallInfoBottomCell () <UITableViewDelegate, UITableViewDataSource>
...
@implementation JKInstallInfoBottomCell
    ...
    if (indexPath.row == 0) {
        ...
        if (self.hasPayServiceFree) {
            ...
            if (self.txtPaymentUrlS.length != 0) {
                ...
                for (NSInteger i = 0; i < txtPaymentUrlS.count; i++) {
                    ...
                    btn.tag = i;
                    [btn addTarget:self action:@selector(txtPaymentUrlSClick:) forControlEvents:UIControlEventTouchUpInside];
                    [imgV addSubview:btn];
                }
            }
        }
        ...
        if (self.hasPayDepositFree) {
            ...
            if (self.txtPaymentUrlD.length != 0) {
                ...
                for (NSInteger i = 0; i < txtPaymentUrlD.count; i++) {
                    ...
                    btn.frame = CGRectMake(0 , 0, 80, 80);
                    btn.tag = i;
                    [btn addTarget:self action:@selector(txtPaymentUrlDClick:) forControlEvents:UIControlEventTouchUpInside];
                    [imgV addSubview:btn];
                }
            }
        ...
        if (self.txtUrls.length != 0) {
            ...
            for (NSInteger i = 0; i < orderPicArr.count; i++) {
                ...
                btn.tag = i;
                [btn addTarget:self action:@selector(orderPicClick:) forControlEvents:UIControlEventTouchUpInside];
                [imgV addSubview:btn];
            }
        }
...
- (void)orderPicClick:(UIButton *)btn {
    NSArray *orderPicArr = [self.txtUrls componentsSeparatedByString:@","];
...
- (void)txtPaymentUrlDClick:(UIButton *)btn {
    NSArray *txtPaymentUrlD = [self.txtPaymentUrlD componentsSeparatedByString:@","];
    JKShowImagePagesView *sipV = [[JKShowImagePagesView alloc] init];
    sipV.frame = [UIScreen mainScreen].bounds;
    [sipV showGuideViewWithImages:txtPaymentUrlD withTag:btn.tag];
    UIView * keywindow = [[UIApplication sharedApplication] keyWindow];
    [keywindow addSubview: sipV];
}

- (void)txtPaymentUrlSClick:(UIButton *)btn {
    NSArray *txtPaymentUrlS = [self.txtPaymentUrlS componentsSeparatedByString:@","];
    JKShowImagePagesView *sipV = [[JKShowImagePagesView alloc] init];
    sipV.frame = [UIScreen mainScreen].bounds;
    [sipV showGuideViewWithImages:txtPaymentUrlS withTag:btn.tag];
    UIView * keywindow = [[UIApplication sharedApplication] keyWindow];
    [keywindow addSubview: sipV];
}
