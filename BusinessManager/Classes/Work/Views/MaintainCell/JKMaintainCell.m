...
- (void)createUI:(JKTaskModel *)model {
    ...
    bgView.backgroundColor = RGBHex(0x4375c7);
...
//    }];
    ...
    pondNameLb.text = [NSString stringWithFormat:@"运维管家：%@",model.matnerMembName];
    ...
    [pondNameLb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(orderNoLb.mas_bottom).offset(5);
        make.left.equalTo(contentView.mas_left).offset(10);
        make.right.equalTo(contentView.mas_right).offset(-10);
        make.height.mas_equalTo(20);
    }];
    ...
    deviceNoLb.text = [NSString stringWithFormat:@"设备型号：%@",model.repairEqpKind];
