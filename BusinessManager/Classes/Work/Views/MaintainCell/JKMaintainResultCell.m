...
@interface JKMaintainResultCell () <UITableViewDelegate, UITableViewDataSource>
...
- (void)punchBtnClick:(UIButton *)btn {
    ...
    [self.punchBtn setTitleColor:RGBHex(0xaaaaaa) forState:UIControlStateNormal];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.maintainType == JKMaintainIng) {
        if (indexPath.row == 2) {
            return (self.contentBtnTitleArr.count / 2  + self.contentBtnTitleArr.count % 2) * 48;
