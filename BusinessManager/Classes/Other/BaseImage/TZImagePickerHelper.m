...
@property (nonatomic, strong) NSMutableArray *imageDataURL;
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (int i = 0; i<photos.count; i++)
        {
            ...
            [self.imageDataURL removeAllObjects];
            [self.imageDataURL addObject:image];
//            if (self.imageType == JKImageTypeIDFront || self.imageType == JKImageTypeIDBackground) {
//                [self.imageDataURL removeAllObjects];
//                [self.imageDataURL addObject:image];
//            } else {
//                [self.imageDataURL addObject:image];
//            }
            ...
            if (self.imageType == JKImageTypeIDFront) {
                self.finishIDFront(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeIDBackground) {
                self.finishIDBackground(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeAccount) {
                self.finishAccount(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeFarmer) {
                self.finishFarmer(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypePond) {
                self.finishPond(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeRepaire) {
                self.finishRepaire(self.imagesURL, self.imageDataURL);
            }else if (self.imageType == JKImageTypeContract) {
                self.finishContract(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypePay) {
                self.finishPay(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeRecyceAnnex) {
                self.finishRecyceAnnex(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeMaintain) {
                self.finishMaintain(self.imagesURL, self.imageDataURL);
            } else if (self.imageType == JKImageTypeAttachFile) {
                self.finishAttachFile(self.imagesURL, self.imageDataURL);
            }
            self.imagesURL = nil;
        }
    });
...
{
    NSData *data;
    NSString *filePath =  [[NSString stringWithFormat:@"%d.jpg",arc4random()] cacheDic];
...
//    }
    ...
    if (image.size.width > SCREEN_WIDTH || image.size.height > SCREEN_HEIGHT) {
        ...
        if (image.size.width > image.size.height)
        {
            imagesize.width = SCREEN_WIDTH * scale;
            imagesize.height = image.size.height*imagesize.width/image.size.width * scale;
        }
        else
        {
            imagesize.height = SCREEN_HEIGHT * scale;
            imagesize.width = image.size.width*imagesize.height/image.size.height * scale;
        }
    } else {
        self.compressionQuality = 0.5;
    }
...
- (NSMutableArray *)imageDataURL
{
    if (!_imageDataURL) {
        _imageDataURL = [NSMutableArray array];
    }
    return _imageDataURL;
}
