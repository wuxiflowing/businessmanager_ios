...
@implementation JKCustomDateView
...
- (void)initStatus:(NSDictionary*)dic {
    ...
    self.todayArray = [JKFactoryManager yearMonthDayFromDate:[[NSDate alloc] init]];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (pickerView.tag == TAGPICKERVIEW) {
        if (component == 0) {
            if ([_leftStartDate isKindOfClass:[NSDate class]] && [_leftEndDate isKindOfClass:[NSDate class]]) {
                NSDateComponents *components = [JKFactoryManager dateComponentForTimeInterval:[_leftStartDate timeIntervalSince1970]];
                self.yearL = row + components.year;
            }else {
                self.yearL = row + yearNow;
            }
        ...
        NSDate *date = [JKFactoryManager dateFromStringByHotlineWithoutSeconds:[[NSString alloc] initWithFormat:@"%@-%@-%@ 00:00",@(self.yearL).stringValue,[self transformStringWithNum:self.monthL],[self transformStringWithNum:self.dayL]]];
        if (self.leftStartDate) {
            BOOL isEarlier = [JKFactoryManager compareDateOne:date earlierToDataTwo:self.leftStartDate];
            
            if (isEarlier) {
                NSArray *minArray = [JKFactoryManager yearMonthDayFromDate:self.leftStartDate];
                if (minArray) {
        ...
        date = [JKFactoryManager dateFromStringByHotlineWithoutSeconds:[[NSString alloc] initWithFormat:@"%@-%@-%@ 00:00",@(self.yearL).stringValue,[self transformStringWithNum:self.monthL],[self transformStringWithNum:self.dayL]]];
        if (self.leftEndDate) {
            BOOL isEarlier = [JKFactoryManager compareDateOne:self.leftEndDate earlierToDataTwo:date];
            
            if (isEarlier) {
                NSArray *minArray = [JKFactoryManager yearMonthDayFromDate:self.leftEndDate];
                if (minArray) {
                    NSInteger year = [minArray[0] integerValue];
                    NSInteger month = [minArray[1] integerValue];
                    NSInteger day = [minArray[2] integerValue];
        ...
        NSDate *date = [JKFactoryManager dateFromStringByHotlineWithoutSeconds:[[NSString alloc] initWithFormat:@"%@-%@-%@ 00:00",@(self.yearR).stringValue,[self transformStringWithNum:self.monthR],[self transformStringWithNum:self.dayR]]];
        if (self.rightStartDate) {
            BOOL isEarlier = [JKFactoryManager compareDateOne:date earlierToDataTwo:self.rightStartDate];
            if (isEarlier) {
                NSArray *minArray = [JKFactoryManager yearMonthDayFromDate:self.rightStartDate];
                if (minArray) {
                    NSInteger year = [minArray[0] integerValue];
                    NSInteger month = [minArray[1] integerValue];
                    NSInteger day = [minArray[2] integerValue];
        ...
        date = [JKFactoryManager dateFromStringByHotlineWithoutSeconds:[[NSString alloc] initWithFormat:@"%@-%@-%@ 00:00",@(self.yearR).stringValue,[self transformStringWithNum:self.monthR],[self transformStringWithNum:self.dayR]]];
        if (self.rightEndDate) {
            BOOL isEarlier = [JKFactoryManager compareDateOne:self.rightEndDate earlierToDataTwo:date];
            if (isEarlier) {
                NSArray *minArray = [JKFactoryManager yearMonthDayFromDate:self.rightEndDate];
                if (minArray) {
                    NSInteger year = [minArray[0] integerValue];
                    NSInteger month = [minArray[1] integerValue];
                    NSInteger day = [minArray[2] integerValue];
    ...
    JKDate *dateL = [[JKDate alloc] init];
    dateL.year = _yearL;
    dateL.month = _monthL;
    dateL.day = _dayL;
    
    JKDate *dateR = [[JKDate alloc] init];
    dateR.year = _yearR;
    dateR.month = _monthR;
    dateR.day = _dayR;
...
- (NSArray *)dayTitlesByYear:(NSInteger)year month:(NSInteger)month {
    NSInteger days = 0;
    BOOL isLeapYear = [JKFactoryManager isLeapYear:year];
    BOOL isBigMonth = NO;
    if (month <=7) {
        if (month % 2 == 1) {
            isBigMonth = YES;
...
- (NSArray *)yearTitles {
    NSMutableArray *array = [[NSMutableArray alloc] init];
    if ([_leftStartDate isKindOfClass:[NSDate class]] && [_leftEndDate isKindOfClass:[NSDate class]]) {
        NSDateComponents *minComponents = [JKFactoryManager dateComponentForTimeInterval:[_leftStartDate timeIntervalSince1970]];
        NSDateComponents *maxComponents = [JKFactoryManager dateComponentForTimeInterval:[_leftEndDate timeIntervalSince1970]];
        ...
        NSInteger yearNow = [self.todayArray[0] integerValue];
        for (int x = 0; x < 10; x ++) {
            [array addObject:@(x + yearNow).stringValue];
        }
    }
    return array;
}
